...
const items = [
  {
    ...
    message: '相見積りでは受託側業者は価格競争で勝つために無理な開発工数の算出で契約を取ろうとする場合が少なくともありません。\n' +
        '結果、実際に<span>作りたかったシステムにならない</span>又は開発途中で<span>追加の費用を請求される</span>事例が多々あります。' +
        '本サービスでは最適な業者を選定判断を行うアドバイスを行います。\n' +
        '結果、プロジェクトの成功率向上に寄与します。'
  },{
    images:{
      webp:webpHm2,
      img:imgHm2,
    ...
    message: '<span>「システム開発業者に依頼して開発をすすめてもらっておけばいいのでは」</span>と思うでしょうがそれは間違いです。' +
        '受託側のゴールは委託者に納品することですが、委託者は完成物を運用していく事も考えなければなりません。\n' +
        'しかし、委託者の技術知識の有無によっては運用も完全委託となり"想定コストが増加する"傾向にあります。' +
  ...
  },
  {
    ...
    title: 'エンジニアに開発をお願いしているが専門用語が多く理解に苦しむ',
    message: 'エンジニアは、専門用語を利用した会話をしがちです。\n' +
        '本サービスはエンジニアの話している内容をわかりやすく要約してお伝えさせていただきます。\n' +
        'これにより委託者、<span>受託者とのコミュニケーションが円滑になる</span>事により、より良いプロジェクト進行に寄与します。'
  },
  {
    images:{
      webp:webpHm3,
    ...
    message: '本サービスでは、プロジェクトに必要な技術選定からご相談いただけます。\n' +
